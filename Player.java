...
class Player {
  ...
  public void aiTakeTurn(Player p1) { // Method to determine how the monster will attack
    if (!this.playerControlled) {
      ...
      // Iteration through monsters so that each of them can attack
      while (iter.hasNext()) {
        ...
        // If the random number is 0 then pick a random index from the list of opponents and attack
        if (randomNumber == 0) {
          int randomIndex = rand.nextInt(p1.getTeam().size());
          pFinal = p1.getTeam().get(randomIndex);
        }

        // If the random number is 1 then target opponent based on damage they do or health they have
        else {
          while (iter2.hasNext()) {
            ...
            // If the party member has less than 20 health, then they are selected to be attacked
            if (p.getHealth() < 20) {
              pFinal = p;
            }

            // If no one is below 20 hp
            else {

              // If party member's attack is greater than the ones before it
              if (p.getDamage() > highestDamage) {
              ...
              // If the damage of the party member is equal to the highestDamage
              else if (p.getDamage() == highestDamage) {

                // Then compare to see if which one of the high damage party members has more health
                if (p.getHealth() > highestHealth) {
                  highestHealth = p.getHealth(); // Set highest health to the party member's health
  ...
  public void userTakeTurn(Player p1, Player p2) {
    ...
    for (int x = 0; x < (p1.getTeam()).size(); x++) { // Iteration through party members so that each of them can attack
      ...
      System.out.println(YELLOW + "\n" + p.getName() + CYAN + " | " + PURPLE + "Level: " + p.getLevel() + CYAN + " | "
          + RED + "Health: " + p.getHealth() + GREEN);
      ...
      if (actionSelection.equals("1")) { // Player choose to attack
        ...
        if (p.getName().contains("Barbarian")) { // Barbarian Attack
          ...
          // If the player inputs anything not an int, they are prompted to select a number
          catch (Exception ex) {
        ...
        if (p.getName().contains("Wizard")) {
          ...
          System.out.println(GREEN
              + "Do you want to target a single enemy or attack the entire party?\n(1 = Single Enemy | 2 = Party)");
      ...
      else if (actionSelection.equals("4")) {
